...
public struct AirForce
{
    ...
    /// 从字节数组解析空军数据
    /// </summary>
    public static AirForce FromBytes(ReadOnlySpan<byte> data, int offset) =>
        MemoryMarshal.Read<AirForce>(data[offset..]);
    ...
    /// 创建默认空军
    /// </summary>
    public static AirForce CreateDefault() => new AirForce();
