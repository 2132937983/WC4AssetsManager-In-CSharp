...
public sealed class ArmyV3Modifier : ModifierBase
{
    public override string Name => "army_v3";
    public override string DisplayName => "v3单位修改器";

    private Army_3? _copiedArmy;
    ...
    public override ModifierResult Apply(int col, int row, object? parameter = null)
    {
        ...
        {
            army = Army_3.CreateDefault(coordIndex);
        ...
        int idx = _mapData.FindArmyV3Index(col, row);
        if (idx >= 0)
            _mapData.ReplaceArmyV3(idx, army);
        else
            _mapData.ArmiesV3.Add(army);

        MarkModified();
        return ModifierResult.Ok($"已放置v3单位 ({col}, {row})");
    }
    ...
    public override ModifierResult Remove(int col, int row)
    {
        ...
        int idx = _mapData.FindArmyV3Index(col, row);
        if (idx < 0) return ModifierResult.Fail("该位置没有v3单位");

        _mapData.RemoveArmyV3At(idx);
        MarkModified();
        return ModifierResult.Ok($"已删除v3单位 ({col}, {row})");
    }
    ...
    public override bool CanRemove(int col, int row)
    {
        ...
        return _mapData?.FindArmyV3Index(col, row) >= 0;
    }
    ...
    public ModifierResult CopyArmy(int col, int row)
    {
        ...
        int idx = _mapData!.FindArmyV3Index(col, row);
        if (idx < 0) return ModifierResult.Fail("该位置没有v3单位");
        _copiedArmy = _mapData.ArmiesV3[idx];
        return ModifierResult.Ok("已复制v3单位数据");
    }

    public ModifierResult PasteArmy(int col, int row)
    {
        if (!IsValidCoord(col, row)) return ModifierResult.Fail("坐标超出范围");
        if (_copiedArmy == null) return ModifierResult.Fail("没有已复制的v3单位数据");
    ...
    public ModifierResult UpdateArmy(Army_3 army)
    {
        if (_mapData == null) return ModifierResult.Fail("地图数据未初始化");

        var coord = HexCoord.FromIndex(army.Coordinate, _mapData.MapWidth);
        int idx = _mapData.FindArmyV3Index(coord.Col, coord.Row);
        if (idx >= 0)
            _mapData.ReplaceArmyV3(idx, army);
        else
            _mapData.ArmiesV3.Add(army);
        ...
        MarkModified();
        return ModifierResult.Ok("已更新v3单位数据");
    }
    ...
    public ModifierResult SetLegionId(int col, int row, int legionId)
    {
        ...
        var army = _mapData.ArmiesV3[idx];
        army.LegionId = legionId;
        _mapData.ReplaceArmyV3(idx, army);
        MarkModified();
        return ModifierResult.Ok($"已设置v3单位军团ID: {legionId}");
    }
    ...
    public ModifierResult DeleteAllArmies()
    {
        ...
        int count = _mapData.ArmiesV3.Count;
        _mapData.ArmiesV3.Clear();
        MarkModified();
        return ModifierResult.Ok($"已删除所有v3单位，共 {count} 个");
    }
    ...
    public ModifierResult DeleteArmiesByBelong(int belongValue)
    {
        ...
        int deletedCount = 0;
        for (int i = _mapData.ArmiesV3.Count - 1; i >= 0; i--)
        {
            if (_mapData.ArmiesV3[i].LegionId == belongValue)
            {
                _mapData.RemoveArmyV3At(i);
                deletedCount++;
            }
        }

        MarkModified();
        return ModifierResult.Ok($"已删除 {deletedCount} 个归属为 {belongValue} 的v3单位");
    }
    ...
    public ModifierResult EraseArmyBrush(int centerCol, int centerRow, int radius)
    {
        ...
        for (int rowOffset = -radius; rowOffset <= radius; rowOffset++)
        {
            for (int colOffset = -radius; colOffset <= radius; colOffset++)
            {
                ...
                int idx = _mapData.FindArmyV3Index(targetCol, targetRow);
                if (idx >= 0)
                {
                    _mapData.RemoveArmyV3At(idx);
                    deletedCount++;
        ...
        MarkModified();
        return ModifierResult.Ok($"橡皮擦删除了 {deletedCount} 个v3单位");
    }
    ...
    public ModifierResult ModifyLegionStrength(int belongValue, int levelId)
    {
        ...
        // 对齐 VB ModifyLegionStrength：军团列表 v1/v3 共用，同样要写回军团自身的 Rate/Blood
        if (belongValue >= 0)
        {
        ...
        MarkModified();
        return ModifierResult.Ok($"已修改 {modifiedCount} 个v3单位的强度 (等级: {config.Name})");
    }

    public void ApplyLegionLevelConfig(ref Army_3 army, LegionLevelConfig config)
    {
        ...
        if (army.General > 0)
            army.Rank = (byte)_random.Next(config.GeneralHpMin, config.GeneralHpMax + 1);
    }

    /// <summary>
    /// 按 actionId 写回军团自身的血量/税率系数。
    /// Legion 是 struct，必须经 ReplaceLegion 写回，直接改副本会被静默丢弃。
    /// </summary>
    ...
    public ModifierResult AutoAdjustLegionStrengthByCountry(int targetBelongValue)
    {
        ...
        for (int legionIndex = 0; legionIndex < _mapData.Legions.Count; legionIndex++)
        {
            ...
            // 原地写回军团自身的 Rate/Blood（v1/v3 共用同一份军团列表）
            legion.CountryHpRate = (float)config.Blood;
            legion.CountryTaxRate = (float)config.Rate;
        ...
        MarkModified();
        return ModifierResult.Ok($"已自动调整 {processedCount} 个军团的v3单位强度，共修改 {totalModifiedCount} 个单位");
    }
    ...
    public ModifierResult RandomizeLegionArmies(int belongValue)
    {
        ...
        MarkModified();
        return ModifierResult.Ok($"已随机化 {modifiedCount} 个v3单位");
    }

    public void RandomizeArmyData(ref Army_3 army, int terrainType, List<int> armyOnLand, List<int> armyOnSea)
    {
        ...
        if (army.General > 0)
            army.Rank = (byte)_random.Next(1, 4);
    }
    ...
    public ModifierResult AutoAssignGeneralsToArmies3(int targetBelongValue, int requestedGeneralCount, bool clearExisting, bool adaptToUnits)
    {
        if (_mapData == null) return ModifierResult.Fail("地图数据未初始化");
        if (_mapData.ArmiesV3.Count == 0) return ModifierResult.Fail("没有v3单位需要分配将领");
        ...
        var targetArmies = new List<Army_3>();
        for (int i = 0; i < _mapData.ArmiesV3.Count; i++)
        {
            var army = _mapData.ArmiesV3[i];
            int armyBelongValue = _mapData.GetBelongValueByIndex(army.Coordinate);
            ...
            else if (army.General == 0)
                targetArmies.Add(army);
        }

        if (targetArmies.Count == 0)
            return ModifierResult.Fail($"没有找到归属值为 {targetBelongValue} 的v3单位");
        ...
        var armiesByBelong = new Dictionary<int, List<Army_3>>();
        foreach (var army in targetArmies)
        {
            ...
            if (!armiesByBelong.ContainsKey(armyBelongValue))
                armiesByBelong[armyBelongValue] = new List<Army_3>();
        ...
        foreach (var belongPair in armiesByBelong)
        {
            ...
            if (adaptToUnits)
            {
                var armyGroups = GroupArmies3BySpecialty(belongArmies);
                assignedCount = 0;
                assignedCount += AssignGeneralsToArmies3ByUnitType(availableGenerals, armyGroups.Infantry, "Infantry", actualGeneralCount - assignedCount);
                assignedCount += AssignGeneralsToArmies3ByUnitType(availableGenerals, armyGroups.Armor, "Armor", actualGeneralCount - assignedCount);
                assignedCount += AssignGeneralsToArmies3ByUnitType(availableGenerals, armyGroups.Artillery, "Artillery", actualGeneralCount - assignedCount);
                assignedCount += AssignGeneralsToArmies3ByUnitType(availableGenerals, armyGroups.Navy, "Navy", actualGeneralCount - assignedCount);
                assignedCount += AssignGeneralsToArmies3ByUnitType(availableGenerals, armyGroups.AirForce, "AirForce", actualGeneralCount - assignedCount);
            }
            else
            {
                assignedCount = AssignGeneralsToArmies3Randomly(availableGenerals, belongArmies, actualGeneralCount);
            }
        ...
        MarkModified();
        return ModifierResult.Ok($"已成功分配 {totalAssignedCount} 个将领到v3单位");
    }

    private Army3Groups GroupArmies3BySpecialty(List<Army_3> armies)
    {
        var groups = new Army3Groups();
        var cfg = ConfigManager.Instance;
    ...
    private int AssignGeneralsToArmies3ByUnitType(List<int> availableGenerals, List<Army_3> armies, string specialtyType, int maxCount)
    {
        ...
        foreach (int generalId in shuffledGenerals)
        {
            ...
            {
                army.Rank = (byte)generalSettings.MilitaryRank;
                ApplyGeneralSkills3(ref army, generalSettings);
            }

            cfg.AddAssignedGeneral(generalId);

            int armyIdx = _mapData!.FindArmyV3Index(
                HexCoord.FromIndex(army.Coordinate, _mapData.MapWidth).Col,
                HexCoord.FromIndex(army.Coordinate, _mapData.MapWidth).Row);
            if (armyIdx >= 0)
                _mapData.ReplaceArmyV3(armyIdx, army);
    ...
    private int AssignGeneralsToArmies3Randomly(List<int> availableGenerals, List<Army_3> armies, int maxCount)
    {
        if (armies.Count == 0 || maxCount <= 0) return 0;
        ...
        foreach (int generalId in shuffledGenerals)
        {
            ...
            if (generalSettings != null)
            {
                selectedArmy.Rank = (byte)generalSettings.MilitaryRank;
                ApplyGeneralSkills3(ref selectedArmy, generalSettings);
            }
            ...
            if (armyIdx >= 0)
                _mapData.ReplaceArmyV3(armyIdx, selectedArmy);
    ...
    private static void ApplyGeneralSkills3(ref Army_3 army, GeneralSettings settings)
    {
    ...
    private class Army3Groups
    {
        public List<Army_3> Infantry { get; } = [];
        public List<Army_3> Armor { get; } = [];
        public List<Army_3> Artillery { get; } = [];
        public List<Army_3> Navy { get; } = [];
        public List<Army_3> AirForce { get; } = [];
    }
    ...
    public ModifierResult QuickModifyArmyPlan(int col, int row)
    {
        ...
        MarkModified();
        return ModifierResult.Ok($"已修改v3单位方案: {oldPlan} -> {army.Plan}");
    }

    public ModifierResult QuickModifyArmyPlanForSelected(IEnumerable<Army_3> selectedArmies)
    {
        if (_mapData == null) return ModifierResult.Fail("地图数据未初始化");
        ...
        {
            int idx = _mapData.FindArmyV3Index(
        ...
        MarkModified();
        return ModifierResult.Ok($"已批量修改 {modifiedCount} 个v3单位的方案");
    }
    ...
    public ModifierResult BatchModifyArmyPlanByLegion(int belongValue, int planValue)
    {
        ...
        MarkModified();
        return ModifierResult.Ok($"已修改 {modifiedCount} 个v3单位的方案 (归属={belongValue}, 方案={planValue})");
    }
    ...
    public ModifierResult BatchUpdateArmyPlans(int targetLevel, int targetHpPercent)
    {
        ...
        MarkModified();
        return ModifierResult.Ok($"已更新 {modifiedCount} 个v3单位的方案");
    }
    ...
    public ModifierResult GenerateArmiesByProbability(int belongValue, int probability)
    {
        ...
        for (int hexIndex = 0; hexIndex < totalCells; hexIndex++)
        {
            ...
            {
                ApplyLegionLevelConfig(ref army3, strengthCfg);
            }
            else
            {
                army3.HealthBonus = 100;
        ...
        MarkModified();
        return ModifierResult.Ok($"生成了 {newArmyCount} 个新v3单位");
    }
    ...
    public ModifierResult DeleteArmiesByBelongs(IEnumerable<int> belongValues)
    {
        ...
        for (int i = _mapData.ArmiesV3.Count - 1; i >= 0; i--)
        {
            if (belongSet.Contains(_mapData.ArmiesV3[i].LegionId))
            {
                _mapData.RemoveArmyV3At(i);
                deletedCount++;
            }
        ...
        MarkModified();
        return ModifierResult.Ok($"已删除 {deletedCount} 个指定归属的v3单位");
    }
    ...
    public void RandomizeArmyData(ref Army_3 army, LegionLevelConfig strengthConfig, int terrainType, List<int> armyOnLand, List<int> armyOnSea)
    {
        ...
        if (army.General > 0 && strengthConfig.GeneralHpMax > 0)
            army.Rank = (byte)_random.Next(strengthConfig.GeneralHpMin, strengthConfig.GeneralHpMax + 1);
    }
